...
- Retrieve dump from CI/CD using artifacts generated by one of the export jobs such as `z:db-import:dev1`. This file is called `aegis.sql`
- Start the `aegis_database_1` container
- Run the following command in your local system (with equivalent path to `aegis.sql` file) `docker exec -i aegis_database_1 psql -U postgres -d aegis < aegis.sql`
