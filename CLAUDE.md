...
- **Automerge** (v3 + automerge-repo) is the primary data layer for all collaborative entities. The repo is initialized in `src/index.tsx` with a WebSocket adapter pointed at `/socketAutomerge/`. All entity mutations (mission, EVA, station, POI, traverse, action, rex) go through Automerge; mutation helpers are in `src/client/automerge/`. Selectors in `src/store/selectors.ts` read directly from Automerge doc state (e.g. `selectAsPlannedStations(mission: Mission)`) rather than from Redux.
...
- **Socket.io** runs on **two separate Socket.IO server instances**:
  - **New server** (path `/socket`) — hosts the default AEGIS namespace and the Maestro v2 namespace.
...
- **Automerge repo** network adapter mounts at `/socketAutomerge/` via WebSocket upgrade, using a custom `PostgresStorageAdapter` to persist documents to PostgreSQL.
