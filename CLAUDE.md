...
- **Socket.io** server mounts at `/api/v1/socketio` (websocket transport only) with two namespaces:
  - **Default namespace** (`/`) — handles AEGIS web-client connections: visitor presence, heartbeats (`statusFromServer` every 10s), and `storeUpsert`/`storeDelete` events for non-Automerge entities (Presets, STM Rules, Folders). Handlers are in `src/server/express/sockets.ts`. Several events on this namespace are **deprecated** in favour of the `/maestro` namespace: `maestroJoin`, `storeUpsertForMaestro`.
  - **`/maestro` namespace** — dedicated namespace for the external Maestro/Mission-Control API client. Auth is enforced via EMSS token middleware. Emits `dataAll` (full `Maestro.IAegisEntity` payload) throttled at 500 ms per mission to the room `maestro{missionId}`. Supports RPC-style handlers: `getEverything`, `getMission`, `getReadableEva`, `subscribeToEva`, `rexOverwrite`, etc. Setup in `src/server/express/sockets-maestro.ts`; emission logic in `src/server/express/sockets-maestro-emitters.ts`. **This is the target for all new Maestro integration work** — the legacy default-namespace Maestro events above are being phased out.
- **Automerge repo** network adapter mounts at `/api/v1/socketAutomerge/` via WebSocket upgrade, using a custom `PostgresStorageAdapter` to persist documents to PostgreSQL.
