...
- **Socket.io** server mounts at `/api/v1/socketio` (websocket transport only) with two namespaces:
  ...
  - **`/maestro` namespace** — dedicated namespace for the external Maestro/Mission-Control API client. Auth is enforced via EMSS token middleware. Emits `dataAll` (full `Maestro.AegisSlice` payload) throttled at 500 ms per mission to the room `maestro{missionId}`. Supports RPC-style handlers: `getEverything`, `getMission`, `getReadableEva`, `subscribeToEva`, `rexOverwrite`, etc. Setup in `src/server/express/sockets-maestro.ts`; emission logic in `src/server/express/sockets-maestro-emitters.ts`. **This is the target for all new Maestro integration work** — the legacy default-namespace Maestro events above are being phased out.
