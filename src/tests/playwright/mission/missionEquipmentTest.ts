...
export async function missionEquipment(page: Page): Promise<string> {
  ...
  await expect(page.getByLabel("checkbox", { exact: true }).nth(test1Index)).toBeChecked({
    checked: eq1.singleUse,
  });
  await expect(
    page.getByLabel("Equipment item quantity", { exact: true }).nth(test2Index)
  ).toHaveText(eq2.quantity);
  await expect(page.getByLabel("checkbox", { exact: true }).nth(test2Index)).toBeChecked({
    checked: eq2.singleUse,
  });

  // Edit mission equipment and discard
  await page.getByLabel("Edit", { exact: true }).click();
  await page.mouse.move(0, -100);
  await page.waitForTimeout(1000);
  ...
  // Delete some mission equipment and discard
  await page.getByLabel("Edit", { exact: true }).click();
  await page.mouse.move(0, -100);
  await page.waitForTimeout(1000);
