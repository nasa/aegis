...
describe("REX API Endpoint", () => {
  ...
  let newRex: Rex = generateBlankRex({ name: "Vitest Rex-1", evaUuid: "someEvaUuid" });
  ...
  describe("POST request", () => {
    ...
    test("Update a Rex", async () => {
      newRex.name = "Vitest Test New Rex Modified";
      const requestBody: RexUpsertRequest = {
        socketId: "someSocketId",
        missionId: testMissionIds[0],
        rexes: [newRex],
      ...
      expect(res.statusCode).toBe(200);
      expect(res.body.data[0].name).toEqual("Vitest Test New Rex Modified");
    });
  });
