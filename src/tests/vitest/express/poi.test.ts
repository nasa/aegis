...
describe("Poi API Endpoint", () => {
  ...
  let newPoi: POI = generateBlankPoi({ name: "Vitest Test New Poi" });
  ...
  describe("POST request", () => {
    ...
    test("Update a Poi", async () => {
      newPoi.name = "Vitest New Poi Modified";
      const requestBody: POIUpsertRequest = {
        socketId: "someSocketId",
        missionId: testMissionIds[0],
        pois: [{ ...newPoi }],
      ...
      expect(res.body.data[0].name).toEqual("Vitest New Poi Modified");
    });
  });
...
describe("Auth with emss-token header", () => {
  ...
  const newPoi = generateBlankPoi({ name: "Vitest Test New Poi" });
