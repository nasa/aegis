...
export const createFullTestStore = (): StoreType => {
  ...
  for (let i = 0; i < pois.length; i++) {
    const action = generateBlankAction({ name: `Vitest Action-${i}`, poiUuid: pois[i].uuid });
    action.duration = i + 6;
  ...
  for (let i = 0; i < stations.length; i++) {
    const action = generateBlankAction({ name: "Vitest Action-1", stationUuid: stations[i].uuid });
    action.duration = i + 6;
  ...
  for (let i = 0; i < 6; i++) {
    traverses.push(generateBlankTraverse({ name: `Vitest Traverse-${i + 1}` }));
  }
  for (let i = 0; i < traverses.length; i++) {
    const action = generateBlankAction({
      name: "Vitest Action-1",
      traverseUuid: traverses[i].uuid,
    });
    action.duration = i + 6;
  ...
  const eva2: Eva = generateBlankEVA({ name: "Vitest Eva-2 Planned No Rex" });
  eva2.traverseRate = 2;
  eva2.sequence = [
  ...
  const rex1 = generateBlankRex({ name: "Vitest Rex-1", evaUuid: eva1ForRex.uuid });
  ...
  const stmLevel1_1 = generateBlankStmLvl1({ name: "Vitest STM Level1-1", numbering: "1" });
  const stmLevel2_1 = generateBlankStmLvl2({ name: "Vitest STM Level2-1", numbering: "1" });
  stmLevel2_1.level1Uuid = stmLevel1_1.uuid;
  const stmLevel3_1 = generateBlankStmLvl3({ name: "Vitest STM Level3-1", numbering: "1" });
