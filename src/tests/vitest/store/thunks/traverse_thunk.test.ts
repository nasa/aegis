...
describe("Thunk Traverse Tests", () => {
  ...
  test("thunkSaveTraverse()", async () => {
    const traverse: Traverse = generateBlankTraverse({ name: "Vitest Traverse-1" });
    const traverseModified: Traverse = { ...traverse, name: "Vitest Traverse-1 Modified" };
    ...
    const traverseActionModified = {
      ...traverseAction,
      name: "Vitest Traverse Action Modified",
    ...
    expect(store.getState().traverse.traverses[0].name).toEqual("Lander to Vitest Station-1");
  ...
  test("thunkCancelTraverse()", async () => {
    ...
    const unsavedTraverseAction = generateBlankAction({
      name: "Vitest Traverse Action Unsaved",
      traverseUuid: traverse.uuid,
    });
    const store = createCustomTestStore({
      traverse: {
