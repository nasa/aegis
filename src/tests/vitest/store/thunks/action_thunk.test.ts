...
describe("Thunk Action Tests", () => {
  test("thunkCreateAction()", async () => {
    ...
    const mockSetActionOrderUuids = vi.fn();
  ...
  test("thunkUpdateActionLocation()", async () => {
    //populate the action state in the store
    const action: Action = generateBlankAction({ name: "Vitest Action-1", stationUuid: uuidv4() });
    const store = createCustomTestStore({
  ...
  test("thunkGetHighlightedActions()", async () => {
    ...
    const action1: Action = generateBlankAction({ name: "Vitest Action-1", stationUuid: uuidv4() });
    action1.stmPriorities = { [stmUuid1]: 1, [stmUuid2]: 1 };
    const action2: Action = generateBlankAction({ name: "Vitest Action-1", stationUuid: uuidv4() });
  ...
  test("thunkDeleteActionFromDbAndStore()", async () => {
    //populate the action state in the store
    const testAction: Action = generateBlankAction({ name: "Vitest Action-1" });
