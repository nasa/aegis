...
describe("Action Store Tests with mock store", () => {
  test("Delete actions", () => {
    ...
    expect(store.getState().action.actionsFromDb[0].name).toEqual("Vitest Action-1");
    expect(store.getState().action.actionsFromDb[1].name).toEqual("Vitest Action-3");
  });
});
