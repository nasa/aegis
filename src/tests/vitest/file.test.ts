// Testing file: file.tsx - using vitest
import { deleteFile, listFiles, renameFile, unzip, moveFile } from "server/file/file";
import * as fs from "fs";
import * as path from "path";

const staticTestDir = `${process.env.STATIC_DIR}/vitestTest`;
...
afterAll(async () => {
  ...
  if (fs.existsSync(path.join(staticTestDir, "../static/testZip.zip"))) {
    fs.rmSync(path.join(staticTestDir, "../static/testZip.zip"));
  }
});
...
describe("File Functions", () => {
  describe("Rename File", () => {
    test("File Rename: Success", async () => {
      ...
      const fileRenamed = await renameFile("vitestTest", "test.txt", "testRenamed.txt");
  ...
  describe("List Files", () => {
    test("List Files: Success", async () => {
      const files = await listFiles("vitestTest");
    ...
    test("List Files with a Directory", async () => {
      ...
      const staticTestDir2 = `${process.env.STATIC_DIR}/vitestTest/testDir`;
  ...
  describe("Delete File", () => {
    test("Delete File: Success", async () => {
      ...
      const fileDeleted = await deleteFile("vitestTest/testDeleteMe.txt");
  ...
  describe("Unzip File", () => {
    test("Unzip File: Success", async () => {
      const testZipFile = path.join(__dirname, `./fixtures/static/testZip.zip`);
      fs.copyFileSync(testZipFile, path.join(staticTestDir, "../testUnzip.zip"));

      await unzip("testUnzip.zip", "vitestTest", "testUnzipContents");
    ...
    test("Unzip File: Failure zip file doesn't exist", async () => {
      await expect(
        unzip("testZipDoesNotExist.zip", "vitestTest", "testZipContentsDoesNotExist")
      ).rejects.toThrow();
  ...
  describe("Move File", () => {
    test("Move File: Success", async () => {
      ...
      await moveFile("testUnzip.zip", "vitestTest", "testUnzipContents");
      expect(fs.existsSync(dst)).toBe(true);
      expect(fs.existsSync(src)).toBe(false);

      const deletedFile = await deleteFile("vitestTest/testUnzipContents/testUnzip.zip");
      expect(deletedFile).toBe(true);
    });

    test("Unzip File: Failure zip file doesn't exist", async () => {
      await expect(
        moveFile("testZipDoesNotExist.zip", "vitestTest", "testZipContentsDoesNotExist")
      ).rejects.toThrow();
