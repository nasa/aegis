...
import type { MaestroVersionDebugInfo } from "server/maestro/v1/types/socketioMaestro";

// ─── Maegistro v1 namespace connection ────────────────────────────────────────
// v1 lives on the legacy /api/v1/socketio server under the /maestro namespace.
// It stays up until Maestro production clients migrate off v1; this page is the
// only UI for managing / testing v1 traffic.

const createMaestroSocket = (
  serverURL: string,
  emssToken: string
): Socket<MaestroServerToClientEventsV1, MaestroClientToServerEventsV1> => {
  return io(`${serverURL}/maestro`, {
    ...
    autoConnect: false,
  }) as unknown as Socket<MaestroServerToClientEventsV1, MaestroClientToServerEventsV1>;
};
...
const MaestroV1: React.FunctionComponent = () => {
  const navigate = useNavigate();

  // Inspector socket (default namespace on the new /socket server — inspector
  // events are AEGIS-internal, not part of the legacy Maegistro v1 traffic).
  ...
  // Maegistro v1 namespace socket
  const maestroSocket =
    useRef<Socket<MaestroServerToClientEventsV1, MaestroClientToServerEventsV1>>(null);
  ...
  const [joinVisitorName, setJoinVisitorName] = useState<string>("Maestro V1 Monitor Page");
  ...
  // ── rexOverwrite ──────────────────────────────────────────────────────────
  // v1 exposes rexOverwrite directly. v2 replaces it with sendMDAU, so this
  // card only appears on the v1 admin page.
  ...
  // ── Maegistro v1 debug info (visitors, listeners, subscriptions) ─────────
  // Fetched via the v1 /maestro namespace's `getDebugInfo` event — requires
  ...
  const connectAndJoin = () => {
    ...
    sock.on("connect", () => {
      ...
      const maestroVisitor: MaestroVisitorV1 = {
        socketId: socketId,
        name: joinVisitorName.trim() || "Maestro V1 Monitor Page",
    ...
    sock.onAny((event, ...args) => {
      console.log("[maestro v1 socket] received event:", event, args);
    });

    sock.on("connect_error", (err) => {
      console.warn("[maestro v1 socket] connect_error:", err.message);
      setMaestroConnectionStatus("failed");
    });

    sock.on("disconnect", (reason) => {
      console.log("[maestro v1 socket] disconnected:", reason);
  ...
  return (
    <main className={adminCommon.page}>
      <div className={adminCommon.container}>
        ...
        <h1 className={adminCommon.pageTitle}>Maegistro v1 Monitor (Legacy)</h1>
        <p style={{ color: "#94a3b8", marginTop: "-8px", fontSize: "0.9rem" }}>
          Legacy Maegistro v1 traffic on <code>/api/v1/socketio</code> namespace{" "}
          <code>/maestro</code>. Stays up until Maestro production clients migrate to v2.
        </p>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
            ...
            <button
              ...
              title={
                isMaestroConnected
                  ? "Refresh"
                  : "Connect to the v1 maestro namespace to enable refresh"
              }
            >
...
// ─── Maegistro v1 Mission Visitors table ──────────────────────────────────────
...
const PrintMaestroVisitors: FunctionComponent<{
  ...
  if (!rows.length)
    return <div className={adminCommon.emptyState}>No Maegistro v1 visitors connected.</div>;
