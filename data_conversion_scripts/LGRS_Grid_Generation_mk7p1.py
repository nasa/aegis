...
from LGRS_Coordinate_Conversion_mk7p1 import *
...
def initialize_LGRS_grid_generation_globals():
    ...
    LGRS_ACC_pattern = (
        r"(\d+)([A-Z]{1})([A-Z]{1})([A-Z]{1})([-A-Z]{1})(\d+)([-A-Z]{1})(\d+)"
    )
    PolarLGRS_ACC_pattern = (
        r"([A-Z]{1})([A-Z]{1})([A-Z]{1})([-A-Z]{1})(\d+)([-A-Z]{1})(\d+)"
    )
...
def initialize_attribute_table(layer, attribute=True):
    ...
    if attribute:
        ...
        elif (form_out == "LGRS") or (form_out == "PolarLGRS"):
            field_label = ogr.FieldDefn("LGRS", ogr.OFTString)
            layer.CreateField(field_label)

        elif (form_out == "LGRS_ACC") or (form_out == "PolarLGRS_ACC"):
            field_label = ogr.FieldDefn("LGRS_ACC", ogr.OFTString)
            layer.CreateField(field_label)

        elif (form_out == "ACC") or (form_out == "PolarACC"):

            field_label = ogr.FieldDefn("ACC", ogr.OFTString)
            layer.CreateField(field_label)

        else:
            raise ValueError("Coordinate could not be assigned" "attribute table.")
...
def Xport_multipolygons(
    X, Y, GridName, f_o, WKT1, attribute=True, output_file_form="shp", overwrite=False
):
    """
    ...
    if output_file_form == "shp":
        if overwrite:
            if os.path.isfile(f_o + "/" + f_o + ".shp"):
                print(
                    "file:", f_o + "/" + f_o + ".shp", "exists removing and rewriting"
                )
    ...
    # Catch all if the output format could not be determined
        ...
        raise ValueError(
            "Cannot write output file: Output format could " "not be determined"
        )
    ...
    for i in range(0, X.shape[0]):  # change line
        ...
        if attribute:
            ...
            feat = set_field_values(
                feat, k, GridName[i, 0].decode(), X[i, :], Y[i, :], attribute
            )
...
def Xport_polygon(
    ...
    if attribute:
        ...
        # an external function
        feat = set_field_values(
            feat, k, GridName[0, 0].decode(), X[0, :], Y[0, :], attribute
        )
    # assign polygons to feature
    feat.SetGeometry(MultiPolygonString)
    layer.CreateFeature(feat)
...
def Calc_LTM_prime_meridian(zone, type):
    ...
    if type == "rad":
        lam0 = np.deg2rad((zone - 1) * (LTM_Width) - 180.0 + (LTM_Width / 2))
    elif type == "deg":
        lam0 = (zone - 1) * (LTM_Width) - 180.0 + (LTM_Width / 2)
    else:
        raise ValueError(
            "LTM zone prime meridian could not be "
...
def generate_LPS_grid(Grid_Res, h):
    ...
    if Grid_Res == "border":
        ...
        if h == "S":
            # grid-lines W->E
            GridX_T = np.arange(LatLon_W_Meridian, LatLon_E_Meridian, ResX)[
                :, np.newaxis
            ]
            ...
            # draw E->W values
            GridX_B = np.arange(LatLon_E_Meridian, LatLon_W_Meridian, -ResX)[
                :, np.newaxis
            ]
        ...
        # If no zone could be identified
            ...
            raise ValueError(
                "LPS display Generation Terminated, Hemisphere could "
                "not be identified."
            )
    ...
    # rectangular Coordinates
        ...
        if h == "S":
            GridX = np.arange(LatLon_W_Meridian, LatLon_E_Meridian, ResX)[
                :, np.newaxis
            ]  # grid lines
            ...
            raise ValueError(
                "LPS Generation Terminated, Hemisphere and "
...
def generate_LTM_grid(Grid_Res, z=None, h=None):
    """
    ...
    if Grid_Res == "global_all":
        ...
        ArrayX = int(
            4 + (2 * LTM_Width) / ResX + (2 * 82) / ResY
        )  # dimension all points in
        # grid area
    ...
    # converted later to proper exporting format.
        ...
        if h == "N":  # if in the northern hemisphere of zones, phi >=0°
            ...
            raise ValueError("Correct LTM zone dimensions could not " "be determined.")
        ...
        raise ValueError(
            "LTM Generation Terminated, Grid type could " "not be determined."
        )
...
def generate_LGRS_grid(
    ...
    if Grid_Res == "global":  # LGRS 8° Areas in LTM Coordinates
        ...
        if h == "N":  # if in the northern portion
            ...
            raise ValueError(
                "Correct LTM zone dimensions for LGRS grid " "be determined."
            )
    ...
    # grid of 25km grid zone in all LTM zones in LatLon
        ...
        for i in range(int(LTM_S_Border), int(LTM_N_Border), int(LTM_N_Border)):
            for j in range(int(LatLon_W_Meridian), int(LatLon_E_Meridian), LTM_Width):
                ...
                LGRS_25km_geom_single_area = assign_xy_grid_to_multipolygon(
                    Cylindrical_X, Cylindrical_Y
                )
                ...
                for m in range(0, GlobalX.shape[0]):
                    for n in range(0, GlobalY.shape[1]):
                        if np.isnan(GlobalX[m, n]):
                            continue
                        z, h, GlobalX[m, n], GlobalY[m, n] = toLTM(
                            GlobalX[m, n],
                            GlobalY[m, n],
                            zone=None,
                            trunc_val=0,
                ...
                # union areas into
                LGRS_25kmGlobal_geom_single_area = intersect_multipolygons(
                    LGRS_25km_geom_single_area, LGRS_Global_geom
                )
                ...
                LGRS_area_clipped = clip_multipolygon_by_polygon(
                    LGRS_25kmGlobal_geom_single_area, LTM_zone_clip_geom
                )
                ...
                for mi in range(0, GridX.shape[1]):
                    for ni in range(0, GridX.shape[2]):
                        GridX[k, mi, ni], GridY[k, mi, ni] = toLatLon(
                            GridX[k, mi, ni],
                            GridY[k, mi, ni],
                            z,
                            h,
    ...
    # coordinates
        ...
        if lonBand == None or h == None:
            raise ValueError(
                "LGRS 25km single global zone is projected "
    ...
    # plotting smaller grids less than 25km resolution
        ...
        Grid_Res == "1km" or Grid_Res == "100m" or Grid_Res == "10m" or Grid_Res == "1m"
    ):
        ...
        if ulx == None or uly == None or lrx == None or lry == None:
            raise ValueError(
                "LGRS 1km grid requires an input box " "provide ulx,uly,lrx,lry"
            )
        ...
        if not multigrid:
            if z1 != z2 or h1 != h2:
                choice = input(
                    "Map area crosses multiple zones "
                ...
                if choice.lower() != "y":
                    raise ValueError(
                        "Plot Area crosses multiple zones: enable multigrid "
        ...
        for z in zone_range:
            ...
            for h in hems:
                ...
                if z == z1:
                    ...
                    _, _, Cylindrical_Xmin, _ = toLTM(
                        lam0 - 4, 0, zone=None, trunc_val=0, process_errors=False
                    )
                ...
                for i in range(0, GridX.shape[0]):
                    for j in range(0, GridX.shape[1]):
                        GridX[i, j], GridY[i, j] = toLatLon(
                            GridX[i, j], GridY[i, j], zone=z, h=h, process_errors=False
                        )
                ...
                LGRS_Global_geom = assign_arbitrary_xy_to_multipolygon(GlobalX, GlobalY)
                # # union areas into
                LGRS_AreaGlobal_geom_single_area = intersect_multipolygons(
                    LGRS_grid_area, LGRS_Global_geom
                )
                ...
                LGRS_grid_clipped = clip_multipolygon_by_polygon(
                    LGRS_AreaGlobal_geom_single_area, LTM_zone_clip_geom
                )
        ...
        raise ValueError(
            "LGRS Generation Terminated: Grid type could"
            ...
            for i in range(0, GridX.shape[0]):
                for j in range(0, GridY.shape[1]):
                    if np.isnan(GridX[i, j]):
                        continue
                    GridX[i, j], GridY[i, j] = toLatLon(
                        GridX[i, j], GridY[i, j], z1, h, process_errors=True
                    )
        else:
            raise ValueError(
                "LatLon Override could not be applied as "
...
def generate_PolarLGRS_grid(
        ...
        if h == "S":
            ...
            GridX = np.full(
                (
                    2,
                    int(
                        2 * (LPS_S_Border - LPS_S_phi0) / ResY
                        + int(LatLon_E_Meridian - LatLon_W_Meridian / ResX + 3)
                    ),
                ),
                np.nan,
            )
            ...
            GridY_C1 = np.arange(LPS_S_Border, LPS_S_phi0, -ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            GridY_C2 = np.arange(LPS_S_phi0, LPS_S_Border, ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            # Crossing the hemisphere
            GridY_C1 = np.arange(LPS_S_phi0, LPS_S_Border, ResY)[
                :, np.newaxis
            ]  # grid lines
            GridY_C1 = np.append(GridY_C1, LPS_S_phi0)
            ...
            GridY_C2 = np.arange(LPS_S_Border, LPS_S_phi0, -ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            GridX = np.full(
                (
                    2,
                    int(
                        2 * (LPS_N_phi0 - LPS_N_Border) / ResY
                        + int(LatLon_E_Meridian - LatLon_W_Meridian / ResX + 3)
                    ),
                ),
                np.nan,
            ...
            GridY_C1 = np.arange(LPS_N_phi0, LPS_N_Border, -ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            GridY_C2 = np.arange(LPS_N_Border, LPS_N_phi0, ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            # Crossing the hemisphere
            GridY_C1 = np.arange(LPS_N_Border, LPS_N_phi0, ResY)[
                :, np.newaxis
            ]  # grid lines
            GridY_C1 = np.append(GridY_C1, LPS_N_Border)
            ...
            GridY_C2 = np.arange(LPS_N_phi0, LPS_N_Border, -ResY)[
                :, np.newaxis
            ]  # grid lines
        ...
        if h == "S":
            ...
            GridX = np.full(
                (
                    ...
                    int(
                        2 * (LPS_S_Border - LPS_S_phi0) / ResY
                        + int(2 * (LatLon_E_Meridian - LatLon_W_Meridian) / ResX + 4)
                    ),
                ),
                np.nan,
            )
            ...
            # S -> N
            GridY_R = np.arange(LPS_S_phi0, LPS_S_Border, ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            # R -> L
            GridX_T = np.arange(0, LatLon_W_Meridian, -ResX)[
                :, np.newaxis
            ]  # grid lines
            ...
            # N -> S
            GridY_L = np.arange(LPS_S_Border, LPS_S_phi0, -ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            # R -> L
            GridX_T = np.arange(LatLon_E_Meridian, 0, -ResX)[
                :, np.newaxis
            ]  # grid lines
            GridX_T = np.append(GridX_T, LatLon_W_Meridian)
            ...
            GridX = np.full(
                (
                    2,
                    int(
                        2 * (LPS_S_phi0 - LPS_S_Border) / ResY
                        + int(2 * (LatLon_E_Meridian - LatLon_W_Meridian) / ResX + 4)
                    ),
                ),
                np.nan,
            ...
            # S -> N
            GridY_R = np.arange(LPS_N_Border, LPS_N_phi0, ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            # N -> S
            GridY_L = np.arange(LPS_N_phi0, LPS_N_Border, -ResY)[
                :, np.newaxis
            ]  # grid lines
            ...
            raise ValueError(
                "Hemisphere could not be determined for polar" "LGRS global all grid."
            )
    ...
    #  25km grid areas, in a single LPS zone in LPS Coordinates
        ...
        if h == "S":
            ...
            h, X, Y_max = toLPS(-0.0, LPS_S_Border, trunc_val=0)
            ...
            h, X, Y_min = toLPS(-0.0, LPS_N_Border, trunc_val=0)
        ...
        for m in range(0, LPS_X.shape[0]):
            for n in range(0, LPS_X.shape[1]):
                _, LPS_X[m, n], LPS_Y[m, n] = toLPS(
                    LPS_X[m, n], LPS_Y[m, n], trunc_val=0, process_errors=False
                )
        ...
        # clip to bounding box
        LGRS_area_clipped = clip_multipolygon_by_polygon(
            LGRS_25km_geom_polar_area, LPS_zone_clip_geom
        )
    ...
    # generate polar grid in grid sizes less than 25km
        ...
        if h1 != h2:
            raise ValueError(
                "Polar LGRS cannot be generated in both"
                "at the same time:\n Please double check"
        ...
        if ulx == None or uly == None or lrx == None or lry == None:
            raise ValueError(
                "LGRS <1km polar grid requires an input box " "provide ulx,uly,lrx,lry"
            )
        ...
        for i in range(0, GridY.shape[0]):
            for j in range(0, GridY.shape[1]):
                _, Y = toLatLon_Polar(
                    GridX[i, j], GridY[i, j], h1, eqs="Spherical", process_errors=False
                )
                # search to see if any point is outside of the LPS plot
                # area. Set clip to true if true

                if (h1 == "N" and Y < 80) or (h1 == "S" and Y > -80):
                    clip = True
                    break
        ...
        if clip:
            ...
            # clip to bounding box
            LGRS_grid_clipped = clip_multipolygon_by_polygon(
                LGRS_grid_area, LPS_zone_clip_geom
            )
        ...
        raise ValueError(
            "PolarLGRS Generation Terminated: Grid type could"
            "not be determined. Please specify:\n"
            "\{global,global_all,25km,25km_all,1km,100m, "
            "10m, or 1m \}"
        )
    ...
    if LatLon_override:
        if Grid_Res != "global_all":
            for i in range(0, GridX.shape[0]):
                for j in range(0, GridY.shape[1]):
                    GridX[i, j], GridY[i, j] = toLatLon_Polar(
                        GridX[i, j],
                        GridY[i, j],
                        h1,
                        eqs="Spherical",
...
def main(
    ...
    try:
        ...
        if not (
            (form_out == "LTM" and Grid_Res == "global_all")
            or (form_out == "LTM" and Grid_Res == "global")
            or (form_out == "LTM" and Grid_Res == "border")
            or (form_out == "LPS" and Grid_Res == "global")
            or (form_out == "LPS" and Grid_Res == "border")
            or (form_out == "LGRS" and Grid_Res == "global")
            or (form_out == "LGRS" and Grid_Res == "global_all")
            or (form_out == "LGRS" and Grid_Res == "border")
            or (form_out == "LGRS" and Grid_Res == "25km")
            or (form_out == "LGRS" and Grid_Res == "25km_all")
            or (form_out == "LGRS" and Grid_Res == "1km")
            or (form_out == "LGRS" and Grid_Res == "100m")
            or (form_out == "LGRS" and Grid_Res == "10m")
            or (form_out == "LGRS" and Grid_Res == "1m")
            or (form_out == "PolarLGRS" and Grid_Res == "global")
            or (form_out == "PolarLGRS" and Grid_Res == "global_all")
            or (form_out == "PolarLGRS" and Grid_Res == "25km")
            or (form_out == "PolarLGRS" and Grid_Res == "1km")
            or (form_out == "PolarLGRS" and Grid_Res == "100m")
            or (form_out == "PolarLGRS" and Grid_Res == "10m")
            or (form_out == "PolarLGRS" and Grid_Res == "1m")
            or (form_out == "LGRS_ACC" and Grid_Res == "1km")
            or (form_out == "LGRS_ACC" and Grid_Res == "100m")
            or (form_out == "LGRS_ACC" and Grid_Res == "10m")
            or (form_out == "LGRS_ACC" and Grid_Res == "1m")
            or (form_out == "PolarLGRS_ACC" and Grid_Res == "1km")
            or (form_out == "PolarLGRS_ACC" and Grid_Res == "100m")
            or (form_out == "PolarLGRS_ACC" and Grid_Res == "10m")
            or (form_out == "PolarLGRS_ACC" and Grid_Res == "1m")
            or (form_out == "ACC" and Grid_Res == "1km")
            or (form_out == "ACC" and Grid_Res == "100m")
            or (form_out == "ACC" and Grid_Res == "10m")
            or (form_out == "PolarACC" and Grid_Res == "1km")
            or (form_out == "PolarACC" and Grid_Res == "100m")
            or (form_out == "PolarACC" and Grid_Res == "10m")
        ):
            raise ValueError(
                "Output coordinate system and Grid Resolution cannot "
                "be plotted.\n"
        ...
        if not (
            (form_in == "LTM" and form_out == "LTM")
            or (form_in == "LTM" and form_out == "LGRS")
            or (form_in == "LTM" and form_out == "LGRS_ACC")
            or (form_in == "LTM" and form_out == "ACC")
            or (form_in == "LPS" and form_out == "LPS")
            or (form_in == "LPS" and form_out == "PolarLGRS")
            or (form_in == "LPS" and form_out == "PolarLGRS_ACC")
            or (form_in == "LPS" and form_out == "PolarACC")
            or (form_in == "LGRS" and form_out == "LGRS")
            or (form_in == "LGRS" and form_out == "LGRS_ACC")
            or (form_in == "LGRS" and form_out == "ACC")
            or (form_in == "PolarLGRS" and form_out == "PolarLGRS")
            or (form_in == "PolarLGRS" and form_out == "PolarLGRS_ACC")
            or (form_in == "PolarLGRS" and form_out == "PolarACC")
            or (form_in == "LGRS_ACC" and form_out == "LGRS_ACC")
            or (form_in == "LGRS_ACC" and form_out == "ACC")
            or (form_in == "PolarLGRS_ACC" and form_out == "PolarLGRS_ACC")
            or (form_in == "PolarLGRS_ACC" and form_out == "PolarACC")
            or (form_in == "LatLon" and form_out == "LTM")
            or (form_in == "LatLon" and form_out == "LGRS")
            or (form_in == "LatLon" and form_out == "LGRS_ACC")
            or (form_in == "LatLon" and form_out == "ACC")
            or (form_in == "PolarLatLon" and form_out == "LPS")
            or (form_in == "PolarLatLon" and form_out == "PolarLGRS")
            or (form_in == "PolarLatLon" and form_out == "PolarLGRS_ACC")
            or (form_in == "PolarLatLon" and form_out == "PolarACC")
        ):
            raise ValueError(
                "Input coordinate system and Output coordinate system "
                "cannot be processed. Please provide on of the following:\n"
    ...
    try:
        ...
        if form_in == "LatLon":
            if Grid_Res == "global_all" or Grid_Res == "25km_all":
                ...
                if len(sys.argv) - 1 == 5:
                    ...
                    raise IndexError(
                        "Operation aborted: Number of inputs not correct."
        ...
        # Lunar Transverse Mercator
            ...
            if Grid_Res == "global_all" or Grid_Res == "25km_all":
                ...
                if len(sys.argv) - 1 == 5:
                    try:
                        z1, h1, ulx, _, uly, _ = list(
                            re.findall(LTM_pattern_full, sys.argv[4])[0]
                        )
                        z2, h2, lrx, _, lry, _ = list(
                            re.findall(LTM_pattern_full, sys.argv[5])[0]
                        )
                        ...
                        raise IndexError(
                            "Operation aborted: LTM Coordinates could not be"
                ...
                raise IndexError(
                    "Operation aborted: Input Coordinates could not be read"
                )
            ...
            if Grid_Res not in grid_format[5:]:
                if len(sys.argv) - 1 == 4:
                    try:
                        ...
                        raise IndexError(
                            "Operation aborted: LPS Coordinates could not be"
                            " converted to proper format correctly."
                        )
                ...
                if len(sys.argv) - 1 == 5:
                    try:
                        h1, ulx, _, uly, _ = list(
                            re.findall(LPS_pattern_full, sys.argv[4])[0]
                        )
                        h2, lrx, _, lry, _ = list(
                            re.findall(LPS_pattern_full, sys.argv[5])[0]
                        )
                    except:
                        raise IndexError(
                            "Operation aborted: LPS Coordinates could not be"
        ...
        # Lunar Grid Reference System in LTM regions
            ...
            if Grid_Res == "global_all" or Grid_Res == "25km_all":
                ...
                try:
                    ...
                    raise IndexError(
                        "Operation aborted: LGRS Coordinates could not be"
                        " converted to proper format correctly."
                    )
                ...
                if len(sys.argv) - 1 == 15:
                    ...
                    try:
                        lonBand1, latBand1, e25k1, n25k1, E1, N1 = list(
                            re.findall(LGRS_pattern, sys.argv[4])[0]
                        )
                        lonBand2, latBand2, e25k2, n25k2, E2, N2 = list(
                            re.findall(LGRS_pattern, sys.argv[5])[0]
                        )
                    except:
                        raise IndexError(
                            "Operation aborted: Number of inputs not correct."
                ...
                raise IndexError(
                    "Operation aborted: LGRS grid size could not be determined"
        ...
        # Lunar Grid Reference System in LPS regions
            ...
            if Grid_Res not in grid_format[5:]:  # 25km or greater
                ...
                if len(sys.argv) - 1 == 13:
                    ...
                    try:
                        lonBand1, e25k1, n25k1, E1, N1 = list(
                            re.findall(PolarLGRS_pattern, sys.argv[4])[0]
                        )
                        lonBand2, e25k2, n25k2, E2, N2 = list(
                            re.findall(PolarLGRS_pattern, sys.argv[5])[0]
                        )
                    except:
                        raise IndexError(
                            "Operation aborted: Number of inputs not correct."
                ...
                try:
                    ...
                    raise IndexError(
                        "Operation aborted: Polar LGRS Coordinates could not be"
                        " converted to proper format correctly."
                    )

            else:
                raise IndexError(
                    "Operation aborted: PolarLGRS grid size could not be determined. "
                    "Please select one of the following:\n{}".format(grid_format_str)
                )
        ...
        # Lunar Grid Reference System in ACC format in LTM regions
            ...
            if Grid_Res in grid_format[5:]:  # any <= 25km or smaller
                if len(sys.argv) - 1 == 19:
                    ...
                    try:
                        lonBand1, latBand1, e25k1, n25k1, e1k1, E1, n1k1, N1 = list(
                            re.findall(LGRS_ACC_pattern, sys.argv[4])[0]
                        )
                        lonBand2, latBand2, e25k2, n25k2, e1k2, E2, n1k2, N2 = list(
                            re.findall(LGRS_ACC_pattern, sys.argv[5])[0]
                        )
                ...
                try:
                    ...
                    raise IndexError(
                        "Operation aborted: LGRS_ACC Coordinates could not be"
                        " converted to proper format correctly."
                    )

            else:
                raise IndexError(
                    "Operation aborted: Grid size for LGRS in ACC must be <=25km"
        ...
        # Lunar Grid Reference System in ACC format in LTM regions
            ...
            if Grid_Res in grid_format[5:]:  # any <= 25km or smaller
                if len(sys.argv) - 1 == 17:
                    ...
                    try:
                        lonBand1, e25k1, n25k1, e1k1, E1, n1k1, N1 = list(
                            re.findall(PolarLGRS_ACC_pattern, sys.argv[4])[0]
                        )
                        lonBand2, e25k2, n25k2, e1k2, E2, n1k2, N2 = list(
                            re.findall(PolarLGRS_ACC_pattern, sys.argv[5])[0]
                        )
                    except:
                        raise IndexError(
                            "Operation aborted: Number of inputs not correct."
                ...
                try:
                    ...
                    raise IndexError(
                        "Operation aborted: Polar LGRS ACC Coordinates could not be"
                        " converted to proper format correctly."
                    )
    ...
    try:
        if form_in == "LatLon":
            if Grid_Res == "global_all" or Grid_Res == "25km_all":
                ...
                z, h, E, N = toLTM(
                    cent_x, cent_y, zone=None, trunc_val=0, process_errors=True
                )

            elif Grid_Res in grid_format:
                z1, h1, ulx, uly = toLTM(
                    ulx, uly, zone=None, trunc_val=0, process_errors=True
                )
                z2, h2, lrx, lry = toLTM(
                    lrx, lry, zone=None, trunc_val=0, process_errors=True
                )
            else:
                raise ValueError(
                    "Operation aborted: Conversion Method from LatLon to common format failed."
                )
            ...
            if Grid_Res not in grid_format[5:]:  # 25km or greater
                ...
                raise ValueError(
                    "Operation aborted:  Conversion Method from PolarLatLon to common format failed."
                )
            ...
            if Grid_Res not in grid_format[5:]:  # 25km or greater
                ...
                raise ValueError(
                    "Operation aborted: Conversion Method from LGRS to common format failed."
                )
            ...
            if Grid_Res not in grid_format[5:]:  # 25km or greater
                ...
                h1, ulx, uly = LGRStoLPS(
                    lonBand1, e25k1, n25k1, E1, N1, process_errors=True
                )
                h2, lrx, lry = LGRStoLPS(
                    lonBand2, e25k2, n25k2, E2, N2, process_errors=True
                )
            else:
                raise ValueError(
                    "Operation aborted: Conversion Method from Polar LGRS to common format failed."
                )

        elif form_in == "LGRS_ACC":
            if Grid_Res in grid_format:
                z1, h1, ulx, uly = LGRS_ACCtoLTM(
                    latBand1,
                    lonBand1,
                    e25k1,
                    n25k1,
                ...
                raise ValueError(
                    "Operation aborted: Conversion Method from Polar LGRS_ACC to common format failed."
                )

        elif form_in == "PolarLGRS_ACC":
            if Grid_Res in grid_format[5:]:
                h1, ulx, uly = LGRS_ACCtoLPS(
                    lonBand1,
                    e25k1,
                    n25k1,
                    e1k1,
            ...
            raise ValueError(
                "Operation aborted: Conversion Method To Common input format failed."
        ...
        if Grid_Res in grid_format[5:]:
            ...
            if (
                ...
                if ulx_test > lrx_test:
                    ...
                    raise ValueError(
                        "Grid ULX and LRX Eastings are equal, no area to plot"
                    )
            ...
            # test polar areas
                ...
                (form_in == "LPS")
                or (form_in == "PolarLatLon")
                or (form_in == "PolarLGRS")
                or (form_in == "PolarLGRS_ACC")
            ):
    ...
    try:
        ...
        if Grid_Res in grid_format[:5]:
            ...
            if form_out == "LTM":
                ...
                GridX, GridY, multigrid = generate_LGRS_grid(
                    Grid_Res, z, h, LatLon_override=LatLon_override
                )

            elif form_out == "PolarLGRS":
                GridX, GridY = generate_PolarLGRS_grid(
                    Grid_Res, h, LatLon_override=LatLon_override
                )

                pass
            else:
                raise ValueError(
                    "Operation failed: Large scale grid "
        ...
        # plot grids that are < 25km resolution
            ...
            if form_out == "LGRS" or form_out == "LGRS_ACC" or form_out == "ACC":
            ...
            # plot polar grid
                ...
                form_out == "PolarLGRS"
                or form_out == "PolarLGRS_ACC"
                or form_out == "PolarACC"
            ):
                ...
                raise ValueError(
                    "Grid could not dimension. " "Size could not be determined."
                )
        else:
            raise ValueError("Operation failed: Grid could not be " "generated.")
    ...
    try:
        ...
        if Grid_Res == "global":
            ...
            if (form_out == "LTM") or (form_out == "LGRS"):
                for i in range(0, GridX.shape[0]):
                    for j in range(0, GridX.shape[1]):
                        ...
                        # conversion to LTM
                        zone, hem, GridX[i, j], GridY[i, j] = toLTM(
                            GridX[i, j],
                            GridY[i, j],
                            zone=None,
                            trunc_val=0,
            ...
            # convert polar areas
            elif (form_out == "LPS") or (form_out == "PolarLGRS"):
                for i in range(0, GridX.shape[0]):
                    for j in range(0, GridX.shape[1]):
                ...
                raise ValueError(
                    "Output format for grid in global " "format incorrect."
                )
    ...
    try:
        if attribute:
            ...
            if (Grid_Res == "25km") or (Grid_Res == "25km_all"):
                let_len = 25
            elif Grid_Res == "1km":
                let_len = 9
            elif Grid_Res == "100m":
            ...
            for i in range(0, GridX.shape[0]):
                ...
                # grid name for LGRS grid name
                    ...
                    if (
                        multigrid
                        or Grid_Res == "global_all"
                        or Grid_Res == "25km_all"
                        or LatLon_override
                    ):

                        # convert center of grid location to common format
                        z, h, CentX, CentY = toLTM(
                            CentX, CentY, zone=None, trunc_val=0, process_errors=False
                        )
                    ...
                    lonBand, latBand, e25k, n25k, E, N = toLGRS(
                        CentX, CentY, z, h, trunc_val=1, process_errors=True
                    )
                    ...
                    if Grid_Res == "global" or Grid_Res == "global_all":
                        ...
                        GridName[i, 0] = "{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, E[:2], N[:2]
                        )
                    elif Grid_Res == "100m":
                        GridName[i, 0] = "{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, E[:3], N[:3]
                        )
                    elif Grid_Res == "10m":
                        GridName[i, 0] = "{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, E[:4], N[:4]
                        )
                    elif Grid_Res == "1m":
                        GridName[i, 0] = "{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, E[:5], N[:5]
                        )
                    else:
                        raise ValueError(
                            "Error: LGRS Grid coordinate names could not be "
                            "Generated."
                        )
                ...
                # process LGRS names for LGRS in ACC format
                    ...
                    if multigrid or Grid_Res == "global_all" or LatLon_override:
                    ...
                    # calculate coordinate for naming
                    lonBand, latBand, e25k, n25k, e1k, E, n1k, N = toLGRS_ACC(
                        CentX, CentY, z, h, trunc_val=1, ACC=False, process_errors=True
                    )
                    ...
                    if Grid_Res == "global" or Grid_Res == "global_all":
                        ...
                        GridName[i, 0] = "{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, e1k, n1k
                        )
                    elif Grid_Res == "100m":
                        GridName[i, 0] = "{}{}{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, e1k, E[:1], n1k, N[:1]
                        )
                    elif Grid_Res == "10m":
                        GridName[i, 0] = "{}{}{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, e1k, E[:2], n1k, N[:2]
                        )
                    elif Grid_Res == "1m":
                        GridName[i, 0] = "{}{}{}{}{}{}{}{}".format(
                            lonBand, latBand, e25k, n25k, e1k, E, n1k, N
                        )
                    else:
                        raise ValueError(
                            "Error: LGRS ACC Grid coordinate names could not be "
                            "Generated."
                        )
                ...
                # truncated to 10m without global location reference
                    ...
                    e1k, E, n1k, N = toLGRS_ACC(
                        CentX, CentY, z, h, trunc_val=10, ACC=True, process_errors=True
                    )
                    ...
                    if Grid_Res == "1km":
                        ...
                        raise ValueError(
                            "Error: ACC Grid coordinate cannot be a 1m resolution. Plot "
                            "LGRS_ACC grid instead."
                        )
                    elif Grid_Res in grid_format:
                        raise ValueError(
                            "Error: ACC Grid coordinate specified incorrectly."
                            "Check input grid resolution"
                        )
                    else:
                        raise ValueError(
                            "Error: ACC Grid coordinate names could not be "
                            "generated."
                        )
                ...
                # names for polar LGRS
                    ...
                    lonBand, e25k, n25k, E, N = toLGRS_polar(
                        CentX, CentY, h, trunc_val=1, process_errors=True
                    )
                    ...
                    if Grid_Res == "global" or Grid_Res == "global_all":
                        ...
                        GridName[i, 0] = "{}{}{}{}{}".format(
                            lonBand, e25k, n25k, E[:2], N[:2]
                        )
                    elif Grid_Res == "100m":
                        GridName[i, 0] = "{}{}{}{}{}".format(
                            lonBand, e25k, n25k, E[:3], N[:3]
                        )
                    elif Grid_Res == "10m":
                        GridName[i, 0] = "{}{}{}{}{}".format(
                            lonBand, e25k, n25k, E[:4], N[:4]
                        )
                    elif Grid_Res == "1m":
                        GridName[i, 0] = "{}{}{}{}{}".format(
                            lonBand, e25k, n25k, E[:5], N[:5]
                        )
                    else:
                        raise ValueError(
                            "Error: PolarLGRS Grid coordinate names could not be "
                            "generated."
                        )
                ...
                # Polar LGRS coordinate names in ACC format without truncation
                    ...
                    lonBand, e25k, n25k, e1k, E, n1k, N = toLGRS_polar_ACC(
                        CentX, CentY, h, trunc_val=1, ACC=False, process_errors=True
                    )
                    ...
                    if Grid_Res == "global" or Grid_Res == "global_all":
                        ...
                        GridName[i, 0] = "{}{}{}{}{}".format(
                            lonBand, e25k, n25k, e1k, n1k
                        )
                    elif Grid_Res == "100m":
                        GridName[i, 0] = "{}{}{}{}{}{}{}".format(
                            lonBand, e25k, n25k, e1k, E[:1], n1k, N[:1]
                        )
                    elif Grid_Res == "10m":
                        GridName[i, 0] = "{}{}{}{}{}{}{}".format(
                            lonBand, e25k, n25k, e1k, E[:2], n1k, N[:2]
                        )
                    elif Grid_Res == "1m":
                        GridName[i, 0] = "{}{}{}{}{}{}{}".format(
                            lonBand, e25k, n25k, e1k, E, n1k, N
                        )
                    else:
                        raise ValueError(
                            "Error: PolarLGRS ACC Grid coordinate names could not be "
                            "Generated."
                        )
                ...
                # Polar LGRS coordinate names in ACC format with truncation
                    ...
                    e1k, E, n1k, N = toLGRS_polar_ACC(
                        CentX, CentY, h, trunc_val=10, ACC=True, process_errors=True
                    )
                    ...
                    if Grid_Res == "1km":
                        ...
                        raise ValueError(
                            "Error: Polar ACC Grid coordinate is 1m resolution. Plot "
                            "LGRS_ACC grid instead."
                        )
                    elif Grid_Res in grid_format:
                        raise ValueError(
                            "Error: PolarACC Grid coordinate specified incorrectly."
                            "Check input grid resolution"
                        )
                    else:
                        raise ValueError(
    ...
    try:
        ...
        # export all other grids as a multipolygon
            ...
            raise ValueError(
                "Grid could not be exported. " "Please check function input values "
            )
...
# program
    ...
    grid_res = None
    raise ValueError(
        "Input, output, or grid resolution specified "
        "incorrectly.\n Select on of the following:\n"
        "I/O:\n{}\n\nGrid Res:\n{}".format(coordinate_format_str, grid_format_str)
    )

# run the grid generation program if enough arguments are provided
try:
    if (
        (form_in in coordinate_format)
        and (
            (form_out in coordinate_format)
            or (form_out == "ACC")
            or (form_out == "PolarACC")
        )
        and (grid_res in grid_format)
    ):
