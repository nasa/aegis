...
- ✅ **Scale/z0 (Problem 3, 2026-06-22):** The cap grid's z0 resolution was changed from **8192** to
  **12800** so the tiles match the mission's _existing_ `projResUnitsPerPixel = 12800` (the value the
...
> **Caveat on the body of this doc.** The cap-grid _placement_ work in §2–§7 (cutting tiles on
> origin −931100, the Y-anchor padding) is correct and still required — it controls _where_ tiles land.
...
- The root data-conversion scripts (`raster_to_tiles.py`, `inspect_geotiff.py`,
  ...
  `map-prototype` git branch — so `_main.py` step 3 could not run at all. _(Recovered.)_
...
_image's own corner_ (origin ≈ 94509, z0 = 32) — that was the "wrong scale".
...
(`--min-x/--max-x/--min-y/--max-y` + `--min-zoom/--max-zoom`). Constrain it to _only the tiles the
data covers_, on the full-cap VRT, and it finishes in **~6 s** while still numbering tiles on the
...
3. **The fractional-tile / Y-anchor trap (this is what bit us).** The cap is **not** a whole number
   ...
   **top-left**, drops the partial tile off the **bottom**, then flips Y→TMS by tile _count_. Fed
   the _exact_ cap extent, the two grids disagree by the empty part of that partial tile
...
(The actual _data_ footprint is E 94509.5…99630.5 / N 145483.5…150604.5 — but that is **not** what
...
- Mission 595 defaults to showing only the `nac_sfs_ortho` layer, and **nothing rendered** — the map
  ...
  _zero_ requests). The production basemap (`NAC_POLE_SOUTH_CM_AVG_MERGE`) rendered fine when toggled
...
**Why the basemap worked and the ortho didn't:** identical CRS, origin, and pyramid — the _only_
...
### 8.4 Why this was _not_ a frontend fix

The render path in `leaflet-helper.tsx` is correct _for the AEGIS convention_: every working
...
### 8.5 The §8.3 candidates that were _ruled out_ (left for the record)
...
After §8, the layer's bounds gate passed, so Leaflet finally _requested_ tiles — but they **404'd**,
...
pyramid is per-mission, not per-layer** — you cannot set resolution per layer — so the _tiles_ must
