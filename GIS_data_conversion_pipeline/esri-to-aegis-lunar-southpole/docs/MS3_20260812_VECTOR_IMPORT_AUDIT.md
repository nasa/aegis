...
- The `Translate` interaction is registered in the editor and the dashboard, and skipped on the minimap and while an edit directive is active — the same gate `MarkerLabels` uses. This document specifies editor-only dragging; dashboard dragging is still enabled.
