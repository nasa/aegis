...
# Ensure an SSL cert exists (if not, create self-signed for $(hostname).ndc.nasa.gov)
openssl req -x509 -newkey rsa:4096 \
    -keyout "${PRIVATE_DIR}/nginx.key" \
    -out "${CERTS_DIR}/nginx.crt" \
    -days 365 -nodes \
    -subj "//C=US/C=US/ST=Texas/L=Houston/O=NASA/CN=${CN}" # doubled //C=US/C=US https://github.com/openssl/openssl/issues/8795
